...
class NIHSampleProcessor(DatasetProcessorBase):
    ...
    def get_images(self,
        ...
        normal_df = metadata_df[~metadata_df['Finding Labels'].str.contains("Pneumonia")]
        pneumonia_df = metadata_df[metadata_df['Finding Labels'].str.contains("Pneumonia")]
    ...
    def __get_processed_images(self, df: pd.DataFrame,
                               is_training: bool,
                               output_path: str) -> List[ProcessedImage]:
        ...
        for row in df_list:
            ...
            image_path = (f"{output_path}/sample"
                        + f"/images/{file_name}")
